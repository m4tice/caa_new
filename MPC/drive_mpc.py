...
sys.path.append(r"../../")
from caa.c_utils import data_util as du
from caa.c_utils import dictionaries as dic
from caa.c_utils import sensor_util as su
from caa.c_utils import tools as to
from caa.c_utils import module_p2
from caa.ModelPredictiveControl import cubic_spline_planner as cubic_spline_planner
...
def game_loop(reload=True, hp=False, cp=False):  # hp: Horizon plot - cp: Course plot
    ...
    course = module_p2.get_path(road_segments_file)
    dl = 1.0  # course tick
    ...
    try:
        ...
        # Vehicle
        # start_pose = carla.Transform(carla.Location(x=cx[0], y=cy[0], z=0.500000),
        #                              carla.Rotation(pitch=0.000000, yaw=cyaw[0], roll=0.000000))
        #
        # vehicle = world.spawn_actor(random.choice(blueprint_library.filter('vehicle.tesla.model3')),
        #                             start_pose)

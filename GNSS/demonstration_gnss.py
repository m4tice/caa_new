...
TARGET_SPEED = 9.0 / 3.6  # [m/s] target speed
...
MAX_SPEED = 9.0 / 3.6  # maximum speed [m/s]
...
def game_loop(reload=True, hp=False, cp=False):  # hp: Horizon plot - cp: Course plot
    ...
    try:
        ...
        if cp:
            carla_world_plot(world, course, z=1.5)  # Toggle to plot the
        ...
        while True:
            ...
            try:
                ...
                for _ in range(len(sensor_list)):
                    ...
                    if s_frame[1] == 1:
                        if module_gnss.check_enter_carla_int(vehicle, carla_intersection):
                            print("MPC")
                            vehicle.apply_control(carla.VehicleControl(throttle=ai, steer=di, brake=0))
                        else:
                            print("E2E")
                            v_steer, v_throttle, v_brake = controller.level_one(s_frame[0], camera=True, pp1=True)
                            vehicle.apply_control(carla.VehicleControl(throttle=v_throttle,
                                                                       steer=v_steer,
                                                                       brake=v_brake))
                    else:
...
def main():
    game_loop(reload=True, hp=True, cp=True)
    time.sleep(0.5)
